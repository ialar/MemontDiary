...
from users.views import (LoginRequiredTemplateView, ProfileView,
                         RegisterMessageView, UserCreateView,
                         UserPasswordRecoveryView, UserPasswordSentView,
                         confirm_register)
...
urlpatterns = [
    path(
        "login/",
        LoginView.as_view(template_name="users/login.html", form_class=LoginCustomForm),
        name="login",
    ),
    ...
    path(
        "user_password_sent/", UserPasswordSentView.as_view(), name="user_password_sent"
    ),
