...
def entry_search(request):
    ...
    if query:
        search_list = Entry.objects.filter(title__icontains=query) | Entry.objects.filter(text__icontains=query)
    else:
        search_list = Entry.objects.none()
    return render(request, 'diary/entry_list.html', {'entry_list': search_list, 'query': query})
