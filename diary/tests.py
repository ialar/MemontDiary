...
class EntryTestCase(TestCase):
    def setUp(self):
        ...
        self.entry = Entry.objects.create(
            title="Test Entry 1", text="This is a test entry.", owner=self.user
        )
    ...
    def test_entry_list_view(self):
        ...
        self.assertContains(
            response, "Test Entry 1"
        )  # Проверяем, что запись отображается
    ...
    def test_entry_create_view(self):
        ...
        response = self.client.post(
            ...
            {"title": "New Entry", "text": "This is a new entry.", "is_public": True},
        )
    ...
    def test_entry_detail_view(self):
        ...
        self.assertContains(
            response, self.entry.title
        )  # Проверяем, что заголовок записи отображается
    ...
    def test_entry_search_view(self):
        ...
        Entry.objects.create(
            title="Searchable Entry", text="This entry is searchable.", owner=self.user
        )

        response = self.client.get(
            reverse("diary:entry_search"), {"query": "Searchable"}
        )
        ...
        self.assertContains(
            response, "Searchable"
        )  # Проверяем, что запись отображается в результатах поиска
